...
with sync_playwright() as playwright: #Запуск Playwright в синхронном режиме
    ...
    text_dashboard = page.locator('//h6[@data-testid="dashboard-toolbar-title-text"]')
    expect(text_dashboard).to_be_visible()  #Проверяем видимость элемента
    expect(text_dashboard).to_have_text("Dashboard")  #Проверяем текст